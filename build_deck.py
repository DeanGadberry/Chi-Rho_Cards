...
for variant_name in VARIANTS:
    variant_path = Path("output") / variant_name
    if variant_path.exists():
        shutil.rmtree(variant_path)
    variant_path.mkdir(parents=True)

for stale_file in Path("output").glob("card_*.png"):
    stale_file.unlink()
        ...
        for _, row in df.iterrows():
            ...
            for form, clip in (
                ...
                for side in ("right", "left"):
                    ...
                    output_path = Path("output") / variant_name / f"card_{card_id:03d}_{form}_{side}.png"
